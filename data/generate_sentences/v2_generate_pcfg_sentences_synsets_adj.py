...
Adjectives in NPs (new in this version)
-----------------------------------------
  Each grammar template declares an NP rule:
  ...
  Otherwise the NP falls back to just the noun.
    ...
    We treat them as nouns. 
    The switch does not apply there since the phrase only consists of 1
    word anyways.
...
python v2_generate_pcfg_sentences_synsets_adj.py \
...
Remember adjusting the adjective occurrence rate 
--adj_when_available 
and the --n_samples ceiling (how much we sample from an edge) 
...
# Relations whose predicate already asserts a property — never add another
# adjective inside the NP. Safety belt; grammar also enforces this via NP -> N.
...
# ── HasQuality lookup + NP realization ─────────────────────────────────────────
...
def main():
    ...
    # HasQuality is consumed as the adjective lookup source, not as a sentence-generating edge
