...
+ questions (28.06)
+ NO-ANCHORS variant: strips ALL sentence punctuation (. , ; ! ?) from every
  generated sentence. Sentence punctuation is identical across languages after
  token replacement, so it acts as a shared anchor; this variant removes it so
...
# ___ Punctuation stripping (no-anchors variant) ______________________________

# Sentence punctuation ('.', ',', ';', '!', '?') is stripped from both ends of
# every whitespace token. This is safe: synset/conceptnet IDs and grammar
# terminals always start and end with an alphanumeric, so any leading/trailing
# punctuation is a sentence marker, never part of a token's internal structure.
_PUNCT = '.,;!?'


def strip_symbols(sentence):
    """Remove all sentence punctuation, keeping token-internal '.'/':'/'_' intact."""
    tokens = []
...
def generate_sentences(edges, parsed_grammars, switches, n_samples=8,
    ...
    for edge in edges:
        ...
        results.append({
            ...
            'relation': rel,
            'target': tgt,
            'sentences': list({strip_symbols(s) for s in sentences}),
...
def main():
    ...
    p.add_argument('--output',  default='generated_sentence_no_anchors.json')
