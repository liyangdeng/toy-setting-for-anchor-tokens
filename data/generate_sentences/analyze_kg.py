...
def entity_uniqueness(triples):
    section("(2) ENTITY UNIQUENESS PER RELATION  (mask-ENTITY well-posedness)")
    ...
    def per_relation(ctx, idx):
        # ctx key is a 2-tuple; idx tells which element is the relation
        agg = defaultdict(lambda: [0, 0])   # rel -> [unique_ctx, total_ctx]
    ...
    fwd_rel = per_relation(fwd, 1)   # relation is 2nd element of (s,r)
    bwd_rel = per_relation(bwd, 0)   # relation is 1st element of (r,t)

    relc = Counter(r for _, r, _ in triples)
    print(f"{'relation':22s} {'n_triples':>9s}  "
          f"{'fwd: target|s,r':>16s}  {'bwd: source|r,t':>16s}")
    print("-" * 72)
    for rel, n in relc.most_common():
        fu, ft = fwd_rel[rel]
        bu, bt = bwd_rel[rel]
        fpct = f"{fu}/{ft} ({100*fu/ft:.0f}%)" if ft else "-"
        bpct = f"{bu}/{bt} ({100*bu/bt:.0f}%)" if bt else "-"
        print(f"{rel:22s} {n:9d}  {fpct:>16s}  {bpct:>16s}")
    print("\nRead: 'fwd 99%' = masking the TARGET of this relation is well-posed")
    print("99% of the time. High fwd% relations (hypernym, HasProperty, MadeOf)")
    print("are the good mask-entity candidates; hyponym/RelatedTo are not.")
...
def mask_entity_feasibility(triples, min_n=50, min_degree=2):
    ...
    print(f"{'TOTAL (n>=' + str(min_n) + ')':18s} {tot:6d} {ln:9d} {lo:10d} "
          f"{ld:7d} {tu:7d}")
    print(f"\n  --> {tu} mask-entity-usable triples (always masking the 2nd entity).")
