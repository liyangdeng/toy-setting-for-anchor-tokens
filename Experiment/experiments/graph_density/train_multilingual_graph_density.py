#!/usr/bin/env python3
"""Train a small multilingual BERT-style MLM for graph-density experiments."""
...
def build_epoch_sampled_train(train_a, train_b, epochs, epoch_sample_size, seed):
    rng = random.Random(seed + 1000)
    if epoch_sample_size is None:
        per_epoch_a = len(train_a)
        per_epoch_b = len(train_b)
    else:
...
def plot_loss_history(trainer, output_dir):
    ...
    ax.set_title("Graph-density multilingual MLM loss")
...
def main():
    parser = argparse.ArgumentParser(description="Graph-density multilingual MLM training")
    ...
    parser.add_argument("--epoch_sample_size", type=int, help="Total mixed-language sentences sampled per epoch")
    ...
    train_sents, epoch_counts = build_epoch_sampled_train(
        train_a, train_b, args.epochs, args.epoch_sample_size, args.seed
    )
