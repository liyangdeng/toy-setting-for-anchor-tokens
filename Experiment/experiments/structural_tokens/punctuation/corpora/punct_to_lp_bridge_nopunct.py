"""
build_corpus addition for the NOPUNCT setting of the punctuation experiment
...
WHAT DOES IT DO:
runs build_synset_corpus.py, then strips commas and periods from BOTH
corpora (CJK and Hiragana) and from BOTH sides of the parallel JSON,
using create_corpora_punct.py's DROP_TOKENS.
...
HIRA_CORPUS   = "corpus_hiragana_synset.txt"
...
def main():
    ...
    transform_txt_in_place(out / HIRA_CORPUS, MODE)

    parallel_path = out / PARALLEL_NAME
    if parallel_path.exists():
        process_parallel(parallel_path, parallel_path, MODE, BOTH)

    print(f"[nopunct] stripped commas/periods in {out}")
