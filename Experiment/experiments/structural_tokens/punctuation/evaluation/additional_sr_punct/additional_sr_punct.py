...
version 2 for punctuation (to match the absense of ST at eval time in the ST experiment)
...
usage -> run_punct_additional_sr.sh
...
# No punctuation at eval time - to match the ST experiment.
PUNCT_TOKENS = {",", ".", ";", "*"}

def sentence_vec(sentence, vocab, emb_matrix):
    """Mean-pool word embeddings, excluding punctuation and OOV tokens."""
    ids = [
        vocab[t]
        for t in sentence.split()
        if t in vocab and t not in PUNCT_TOKENS
    ]
    if not ids:
