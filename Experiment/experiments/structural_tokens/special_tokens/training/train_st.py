...
--setting
  shared    [CLS]/[SEP] identical in both languages
  none      no [CLS]/[SEP] at all
  disjoint  CJK: [BEG]/[END]   Hiragana: [CLS]/[SEP]

  [MASK] is shared across languages at all times, so this experiment
...
SPECIAL_SETTINGS = {
    'shared':   {'a': {'cls': '[CLS]', 'sep': '[SEP]'},
                 'b': {'cls': '[CLS]', 'sep': '[SEP]'}},
    'none':     {'a': {'cls': None,    'sep': None,},
                 'b': {'cls': None,    'sep': None,}},
    'disjoint': {'a': {'cls': '[BEG]', 'sep': '[END]'},
                 'b': {'cls': '[CLS]', 'sep': '[SEP]'}},
}

ALL_SPECIALS   = ['[PAD]', '[UNK]', '[CLS]', '[SEP]', '[MASK]', '[BEG]', '[END]']
EXTRA_SPECIALS = ['[BEG]', '[END]']   # registered so they're never masked
...
def main():
    ...
    p.add_argument('--setting', choices=list(SPECIAL_SETTINGS))
    p.add_argument('--corpus_a')
    p.add_argument('--corpus_b')
    p.add_argument('--output_dir', default='st_checkpoints')
