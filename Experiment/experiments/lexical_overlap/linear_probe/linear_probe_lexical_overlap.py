...
if __name__ == "__main__":
    ...
    p.add_argument("--final_omitted", default="probing_run/final_omitted.json",
                    help="final_omitted.json from build_probing_corpus.py")
    p.add_argument("--parallel", default="probing_run/final_omitted_corpus/parallel_corpus_synset.json",
                    help="fresh parallel file built FROM final_omitted.json")
    ...
    # 3. Build probing data examples using the tokenizer
    tok_probe = PreTrainedTokenizerFast.from_pretrained(model_dir)
    examples = build_examples(final_omitted, parallel, cjk_dict, hira_dict, tok_probe.mask_token)
    
    print(f"Built {len(examples)} usable mask-entity triples "
          f"(out of {len(final_omitted)} final_omitted entity-track triples)")
