...
set -e
echo "===== STAGE: filter (re-run with fixed MIN_SURVIVORS=20) ====="
...
echo "===== STAGE: linear_probe.py (entity track) -- continues past per-config failures ====="
for cfg in $CONFIGS; do
  ...
  if [ $? -eq 0 ]; then
    ...
    echo "  probe $cfg FAILED (see cfg_$cfg/probe_log.txt) -- continuing"
  fi
done
